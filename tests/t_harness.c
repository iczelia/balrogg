...
const char * xt_tmp(const char * tag) {
  ...
  int i;
  for (i = 0; i < used; i++)
    if (!strncmp(buf[i] + 8, tag, strlen(tag)) && !strcmp(buf[i] + 8 + strlen(tag), ".tmp"))
      return buf[i];
  if (used == 32) FATAL_CODE(BLR_EXIT_INTERNAL, "t_suite: too many scratch names");
  snprintf(buf[used], sizeof buf[used], "t_suite-%.32s.tmp", tag);
  return buf[used++];
}
